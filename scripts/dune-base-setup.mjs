 ...
 *
 * These three were judged too internal for the audience the dashboard is published for — a
 * viewer does not need the V1-vs-V4 contract migration or the relayer-vs-wallet split. The
 * SQL is kept here so the work is not lost and the history stays readable, but on Dune the
 * queries are hidden, and this script neither deploys nor runs them:
 *
 *   - excluded from the deploy loop — a PATCH would put them back in front of readers
 *   - excluded from `dependentQueryIds`, which is what the daily cron executes
 *
 * ⚠️ `is_temp`, not `is_archived`. Archiving only delists a query; its URL still resolves for
 * anyone holding it. `is_private` is the flag that truly hides one, but the Community plan
 * caps private queries (`max_number_of_private_queries_reached`). `is_temp` hides it with no
 * cap. A query cannot be both temp and archived — the DB rejects it with
 * `not_temp_and_archived` — so flip both flags in the same call. See dune/base-chain/README.md.
 *
 ...
 * that made the old refresh cost ~41 credits a day for nothing.
 *
 * To bring one back: set is_temp false on Dune, give it a DESCRIPTIONS entry, add its chart
 * to the dashboard, and remove it from this set — all four, or it half-works.
 */
const RETIRED = new Set(['13_by_service.sql', '14_by_contract.sql', '15_by_rail.sql']);

/**
 * The description Dune shows under each query's title.
 *
 * 🔴 THIS IS VIEWER-FACING COPY, NOT A CODE COMMENT. It renders on the public query page and
 * on the dashboard. Every query here once shared one hardcoded string that read "Source of
 * truth for this SQL is dune/base-chain/ … re-run scripts/dune-base-setup.mjs rather than
 * editing here" — an instruction to a maintainer, shown to every reader of a public
 * dashboard. Keep these a plain sentence about what the numbers mean. Notes for whoever
 * maintains this belong in dune/base-chain/README.md, which no dashboard reader ever sees.
 */
const DESCRIPTIONS = {
  '00_events.sql':
    'Every AbaPay bill payment settled on Base mainnet — one row per payment. Covers both ' +
    'settlement rails and every AbaPay contract deployed on Base, so history stays continuous ' +
    'across redeploys.',
  '10_kpi_summary.sql':
    'Headline totals for AbaPay on Base: payments settled, unique payers, gross USD volume, ' +
    'and typical payment size.',
  '11_daily_volume.sql':
    'Daily payments and gross USD volume on Base, with a running cumulative total.',
  '12_by_token.sql':
    'How AbaPay payments on Base split between the two stablecoins accepted, USDC and USDT.',
  '16_dau_wau.sql':
    'Active payers on Base — daily, weekly and monthly counts of wallets that made at least ' +
    'one payment in each window.',
  '17_new_vs_returning.sql':
    "Each day's payers on Base split into first-time wallets and wallets that had paid before.",
};
...
function writeIds(queries, { quiet = false } = {}) {
  ...
  const payload = {
    _comment:
      ...
      'daily. That list is only the queries WITH A PANEL on the dashboard: the files in RETIRED ' +
      '(13_by_service, 14_by_contract, 15_by_rail) are hidden on Dune (is_temp) and appear in `queries` ' +
      'below but never in dependentQueryIds, because running a query with no panel spends ' +
      'credits updating something nobody can see. Safe to hand-edit only when the queries were ' +
      'created manually on dune.com (free-plan fallback).',
...
async function main() {
  ...
  // `--verify` skips straight to the run-and-check below: re-PATCHing unchanged queries
  ...
  // Retired queries are archived on Dune; running them would both fail and defeat the point.
