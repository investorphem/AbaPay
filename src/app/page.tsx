...
// ⚡ CURRENCY MAPPER FOR INTERNATIONAL DISPLAY ⚡
const getCurrencySymbol = (countryCode: string) => {
    const map: Record<string, string> = {
        GH: "GHS", ZA: "ZAR", US: "USD", GB: "GBP", KE: "KES", UG: "UGX", TZ: "TZS",
        RW: "RWF", GM: "GMD", GN: "GNF", SL: "SLL", LR: "LRD", ZW: "ZWL", ZM: "ZMW",
        MW: "MWK", MZ: "MZN", BW: "BWP", NA: "NAD", SZ: "SZL", LS: "LSL", BI: "BIF",
        ET: "ETB", MG: "MGA", CD: "CDF", AO: "AOA", CM: "XAF", CG: "XAF", CI: "XAF",
        GA: "XAF", SN: "XOF", ML: "XOF", BF: "XOF", NE: "XOF", TG: "XOF", BJ: "XOF", GW: "XOF"
    };
...
export default function Home() {
  ...
  const activeIntlCurrency = getCurrencySymbol(activeCountry.code);
  ...
  const isFormValid = useMemo(() => {
    ...
    if (isInternational) {
        ...
        // VTPASS REQUIRES A VALID EMAIL FOR INTERNATIONAL TRANSACTIONS
  ...
  const processBlockchainPayment = async () => {
    ...
    try {
      ...
      const newTx: any = { 
          ...
          amountNaira: isInternational ? `${activeIntlCurrency} ${displayForeignAmount}` : calculatedNairaAmount, 
  ...
  // ⚡ UPDATED API PARSERS FOR INTERNATIONAL ⚡
  useEffect(() => {
           ...
           <div className="bg-white w-full max-w-md rounded-t-[2.5rem] sm:rounded-[2.5rem] p-6 pb-10 sm:pb-6 shadow-2xl relative animate-in slide-in-from-bottom-10 sm:zoom-in-95 duration-300">
              ...
              <div className="text-center mb-8">
                 ...
                 <h2 className="text-4xl font-black text-slate-900 mb-2">
                    {isInternational ? `${activeIntlCurrency} ${displayForeignAmount}` : `₦${(parseFloat(calculatedNairaAmount || "0") + currentFee).toLocaleString()}`}
                 </h2>
                               ...
                               <div className="relative animate-in zoom-in-95 duration-200 mt-2">
                                  ...
                                  <div className="p-4 rounded-2xl border-2 border-emerald-500 bg-emerald-50 shadow-sm text-left">
                                     ...
                                     <div className="pt-3 mt-2 border-t border-emerald-200/50 flex justify-between items-end">
                                         ...
                                         <p className="font-black text-emerald-600 text-xl">{activeIntlCurrency} {displayForeignAmount}</p>
                                         <p className="text-[10px] text-slate-500 font-bold">{cryptoToCharge} {selectedToken.symbol}</p>
                                      </div>
                                  </div>
                               </div>
                                        ...
                                        <button key={plan.variation_code} onClick={() => setSelectedIntlVariation(plan)} className="p-3 rounded-xl border border-slate-200 bg-white hover:border-emerald-300 transition-all text-left flex justify-between items-center group">
                                          <div>
                                            ...
                                            <p className="text-[9px] text-slate-400 font-bold mt-0.5">
                                                {isFixed ? `Cost: ${cryptoCostEstimate} ${selectedToken.symbol}` : `Rate: ~${cryptoRateEstimate} ${selectedToken.symbol} per ${activeIntlCurrency}`}
                                            </p>
                                          </div>
                                          {/* ⚡ HIDING NGN, SHOWING LOCAL CURRENCY ⚡ */}
                                          <p className="font-black text-emerald-600 text-sm group-hover:scale-110 transition-transform">
                                            {isFixed ? `${activeIntlCurrency} ${foreignAmt.toLocaleString()}` : "Flexible"}
                                          </p>
                                        </button>
                                      )
                                  })}
