-- AbaPay on Celo — new vs returning payers, daily
-- Reads the root query's materialized view; never touches celo.logs. See 00_events.sql.
--
-- "New" is first-ever payment ON CELO. A wallet that has paid on Base before but is
-- paying on Celo for the first time counts as new here, which is the right reading
-- for a Celo-scoped dashboard.
