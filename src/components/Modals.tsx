...
// ⚡ FAQ — the short, in-app answers. The long-form versions live on /docs; this exists so the
// questions people actually ask mid-payment ("why was I refused?", "is my PIN my wallet
// password?") are one tap away from wherever they are, instead of a page navigation away.
// Every answer here is written from real behaviour — if you change a limit, a supported wallet,
// a refund path or a kill switch, update this too (see CLAUDE.md).
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/60 dark:bg-black/80 backdrop-blur-sm flex justify-center items-center p-4 animate-in fade-in transition-colors" onClick={onClose}>
       <div className="bg-white dark:bg-[#111114] w-full max-w-md rounded-[2rem] shadow-2xl dark:shadow-black/50 p-6 flex flex-col max-h-[80vh] animate-in zoom-in-95 duration-200 transition-colors" onClick={(e) => e.stopPropagation()}>
          <div className="flex justify-between items-center mb-4 shrink-0 border-b border-slate-100 dark:border-slate-800/60 pb-4">
            <h2 className="text-xl font-black tracking-tight text-slate-900 dark:text-white">FAQ</h2>
          ...
          <div className="overflow-y-auto text-sm text-slate-600 dark:text-slate-300 space-y-4 pr-2 leading-relaxed">
             <p className="font-bold text-slate-800 dark:text-slate-100">How do I pay a bill?</p>
             <p>Five ways, all of them the same payment engine underneath. In this app: pick the service, enter the number, confirm and sign in your wallet. By chat: message AbaPay on Telegram, WhatsApp or X in plain language ("send ₦500 airtime to 08012345678"). Or through an AI assistant: connect AbaPay as a tool over MCP and ask it to pay. Chat and AI payments need a spending allowance approved first — see below.</p>

             <p className="font-bold text-slate-800 dark:text-slate-100 mt-4">What can I actually pay for?</p>
             <p>Airtime, mobile data, electricity (prepaid and postpaid), cable TV (DStv, GOtv, Startimes), education PINs, bank transfers, and international airtime/data. The provider list you see is pulled live from our payment provider, so it only ever shows what can genuinely be bought right now — if a provider disappears from the list, it isn't purchasable at that moment.</p>

             <p className="font-bold text-slate-800 dark:text-slate-100 mt-4">What if my payment goes through but the bill doesn't deliver?</p>
             <p>You get refunded, and you don't have to chase it. The moment a delivery fails after your money has landed on-chain, the transaction is flagged, queued for refund, and our operators are alerted. You're told on the channel you used and by email if we have one, then again when the refund actually lands. It goes back to the same wallet, in the same stablecoin, and every refund is verified on-chain before it's marked done.</p>
             <p>If the blockchain transaction itself failed, nothing was ever taken from you — there's nothing to refund. Still stuck? Tap Support on the receipt; the ticket carries your transaction hash automatically.</p>

             <p className="font-bold text-slate-800 dark:text-slate-100 mt-4">What wallets can I use?</p>
             <p>MiniPay (Opera Mini's Celo wallet, where gas is paid in stablecoin so you don't need CELO), Valora, MetaMask, Coinbase Smart Wallet / Base Account, and any other WalletConnect wallet. AbaPay also runs as a Farcaster Mini App. Base Account users get sponsored gas — the app detects it and batches approval and payment into one free transaction; everyone else pays normal network fees.</p>

             <p className="font-bold text-slate-800 dark:text-slate-100 mt-4">Which chains and coins?</p>
             <p>Celo and Base. USD₮ and USDC on both; cUSD/USDm on Celo only. The token list filters itself to whatever chain your wallet is on, so you can't accidentally pick one that doesn't exist there.</p>

             <p className="font-bold text-slate-800 dark:text-slate-100 mt-4">What's the difference between paying in the app and letting an agent pay for me?</p>
             <p>In the app, you sign every payment in your wallet — nothing moves without that signature. With an agent, you approve an on-chain <em>spending allowance</em> once from the Agent Hub, for one specific chain and one specific stablecoin. After that, the agent can pay without a fresh signature each time, but only up to what's left of that allowance. The cap is enforced by the smart contract itself, not by our servers, so it cannot reach the rest of your wallet no matter what goes wrong on our side.</p>
             <p>Set the allowance back to zero from the Agent Hub whenever you want; it takes effect on-chain immediately. There are also our own per-transaction and daily limits on top of yours.</p>

             <p className="font-bold text-slate-800 dark:text-slate-100 mt-4">Is my PIN the same as my wallet password?</p>
             <p>No, and it's important not to confuse them. Your wallet password/recovery phrase belongs to your wallet app and we never see it. Your AbaPay PIN is a separate 4–6 digit code you set when linking a chat account or creating an AI-agent key, and it gates one thing: authorising a payment through those channels. It cannot move funds on its own — it only unlocks spending inside the allowance you already approved on-chain. Wrong PINs lock the channel out after a few tries.</p>

             <p className="font-bold text-slate-800 dark:text-slate-100 mt-4">If I connect an AI assistant, does it still need my PIN?</p>
             <p>Yes — every single payment, every time. Signing in to the connector means "this assistant may act for me"; it does not mean "this assistant may spend". Without the PIN, a connected assistant can only read your balance. If something claims it can pay without asking for your PIN, treat that as a red flag.</p>

             <p className="font-bold text-slate-800 dark:text-slate-100 mt-4">Why was my payment refused?</p>
             <p>Usually one of four things. (1) The service or that specific provider is paused — we do that during an outage or a dispute, and we'd rather refuse up front than take your money for something we know is broken. (2) The amount is outside the provider's own limits, which vary by network and biller, not by one flat rule. (3) You picked a provider our payment provider can't currently sell. (4) For agent payments: the amount exceeds your remaining allowance, your daily limit, or our operator cap. The refusal message tells you which one.</p>

             <p className="font-bold text-slate-800 dark:text-slate-100 mt-4">Which countries can I send international airtime to?</p>
             <p>Every country our payment provider currently covers — well over a hundred, including Ghana, Kenya, South Africa, the UK and the US. We read that list live rather than keeping our own copy, so what the app offers is exactly what can be delivered today. Pick the country in the app (or just name it in chat) and you'll see its own operators, plans and local currency.</p>

             <p className="font-bold text-slate-800 dark:text-slate-100 mt-4">Is JAMB supported?</p>
             <p>Not right now, honestly. WAEC result-checker and WAEC registration PINs work in the app, in chat and through an AI agent. JAMB is fully built on our side, but it isn't enabled on our current merchant account with our payment provider, so it doesn't appear in the live product list and can't be bought. If that changes on their side, it will appear on its own.</p>

             <p className="font-bold text-slate-800 dark:text-slate-100 mt-4">Do you hold my money?</p>
             <p>No. There's no AbaPay balance to top up. Your stablecoins stay in your own wallet until the moment you pay a bill, and they go straight into the payment smart contract — we never hold your keys and can't move your funds without your signature or your explicitly approved allowance.</p>

             <p className="font-bold text-slate-800 dark:text-slate-100 mt-4">Where do I get my electricity token or education PIN?</p>
             <p>On the receipt, immediately after a successful payment — it's shown in full and can be shared as an image or saved as a PNG/PDF. It's also in your History tab, and sent by SMS/email where we have those details.</p>
             ...
             <p className="text-xs bg-slate-50 dark:bg-[#1a1a1f] border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 rounded-xl p-3 font-medium mt-4">
               Longer explanations live on the <a href="/docs" className="underline font-bold">Docs page</a>. Anything else: <a href="mailto:support@abapays.com" className="underline font-bold">support@abapays.com</a>.
             </p>
          </div>
       </div>
    </div>
