...
export default function Home() {
  ...
  const dynamicMinAmount = useMemo(() => {
    ...
    if (activeService.id === "BANK") return 1000; 
    return 100; 
  }, [activeService]);
    ...
    if (activeService.id === "BANK") {
      const fetchBanks = async () => {
        ...
        try {
          ...
          if (data?.content?.varations) banksArr = Array.isArray(data.content.varations) ? data.content.varations : Object.values(data.content.varations);
          else if (data?.content?.variations) banksArr = Array.isArray(data.content.variations) ? data.content.variations : Object.values(data.content.variations);

          if (banksArr && banksArr.length > 0) {
      ...
      const fetchInternetVariations = async () => {
        ...
        try {
          ...
          const data = await res.json();
          let arr = extractVtpassArray(data);
          if (data?.content?.varations) arr = Array.isArray(data.content.varations) ? data.content.varations : Object.values(data.content.varations);
          setInternetVariations(arr);
        } catch (e) {}
      };
  ...
  // ⚡ RESTORED THE COUNTRY SELECTOR FUNCTION ⚡
