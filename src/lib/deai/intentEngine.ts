...
15. CHAIN/TOKEN OVERRIDE — a user can name a specific blockchain and/or stablecoin, either
   ...
   Valid chain values: "CELO" | "BASE". Valid token values: "USDC" | "USD₮" | "USA₮" (USA₮ is
...
// USA₮ replaced USDm as Celo's third payment token (USDm has no EIP-3009 so it could never
  ...
  const cleaned = raw
    ...
    .map((r: any) => {
      ...
      return {
        ...
        // USA₮ is Celo-only — an impossible pairing means the model got it wrong; drop the
        // override entirely rather than propagate a request that can never be fulfilled.
        chain, token: token === 'USA₮' && chain === 'BASE' ? null : token,
      };
    })
