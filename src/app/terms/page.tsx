        ...
        <div className="space-y-8 text-sm sm:text-base text-slate-600 dark:text-slate-300 leading-relaxed font-medium">
          ...
          <section>
            ...
            <p className="mb-3">
              ...
              <strong> Base, Celo, and other supported EVM Blockchains</strong> to exchange digital assets (such as cUSD, USDC, or USDT) for fiat-denominated utility services (Airtime, Data, Electricity, Cable TV, Education PINs, and Bank Transfers) across Nigeria and supported international countries. The specific providers and services available at any time are sourced live from our utility aggregator and may change without notice if a provider becomes unavailable.
            </p>
            <p className="mb-3">
            ...
            <p>
              Payments may be initiated directly through the App, through our conversational AI agent ("DeAI") on Telegram, WhatsApp, X, and the in-app chat widget, or through a third-party AI agent (such as Anthropic's Claude) connected via our <strong>MCP (Model Context Protocol) server</strong> — see <strong>Section 7</strong> below for the specific terms governing agent-initiated payments across all of these. Depending on the asset and network, on-chain settlement may occur either through a direct smart contract call or via the <strong>x402</strong> HTTP-payment protocol; both routes deposit funds into the same audited smart contract vault and carry identical obligations and protections under these Terms.
            </p>
          </section>
          ...
          <section className="bg-indigo-50/50 dark:bg-indigo-900/10 p-6 rounded-2xl border border-indigo-100 dark:border-indigo-900/30 transition-colors">
            ...
            <p className="mb-3">
              AbaPay provides an AI-powered conversational agent ("<strong>DeAI</strong>") reachable via Telegram, WhatsApp, X, and an in-app chat widget, which lets you check balances and pay bills using natural language instead of the web interface. AbaPay also operates an <strong>MCP (Model Context Protocol) server</strong>, which lets a third-party AI agent you use directly — such as Anthropic's Claude — check your balance and pay bills on your behalf once you have connected it. By linking a messaging account, connecting an MCP client, or using DeAI in any form, you agree to the following, in addition to the rest of these Terms:
            </p>
            <ul className="list-disc pl-5 space-y-2 mb-3">
              ...
              <li><strong>Your linked messaging account, connected MCP client, and Transaction PIN are your responsibility.</strong> If a third party gains access to your Telegram, WhatsApp, or X account, your authorized MCP connection, or learns your PIN, they may be able to instruct the agent to spend from your approved allowance up to its bounds. AbaPay is not liable for losses resulting from a compromised messaging account, AI agent account, device, or PIN. Treat your PIN like a banking PIN, and revoke your allowance immediately if you suspect unauthorized access.</li>
              <li><strong>MCP access specifically.</strong> Connecting an MCP client either uses an API key you generate in the Agent Hub, or an OAuth authorization you grant once in your browser (after which the client stays connected without re-entering credentials). Either way, every individual payment still requires your Transaction PIN — OAuth authorizes the connection, never a specific spend. You may revoke an MCP API key or OAuth connection at any time from the Agent Hub.</li>
              ...
              <li><strong>Operator controls.</strong> We reserve the right to pause, rate-limit, or disable agent-initiated payments (in whole, per-channel, or per-service) at any time, including via an emergency kill switch, without prior notice, for security, regulatory, or operational reasons.</li>
