...
//
// Zero npm dependencies, deliberately — Node's built-in `http` and `https` are enough for a
// JSON-RPC relay this small, and it keeps the Dockerfile below to two lines with no install
// step at all.

const http = require('http');
const https = require('https');

const PROD_MCP_URL = 'https://www.abapays.com/api/mcp';
...
const TOOLS = [
  ...
  { name: 'check_balance', title: 'Check Balance', description: "Check a linked wallet's stablecoin balances and remaining agent spending allowance. Works with no arguments once authorized via OAuth; otherwise pass the api_key.", inputSchema: { type: 'object', properties: { api_key: { type: 'string', description: 'AbaPay MCP API key (starts with aba_mcp_). Not needed over OAuth.' }, chain: { type: 'string', enum: ['CELO'], description: 'Which chain to check.' } }, required: [], additionalProperties: false } },
...
function proxyToProduction(bodyStr, res) {
  ...
  const req = https.request(
    ...
    (prodRes) => {
      ...
      prodRes.on('end', () => sendJson(res, prodRes.statusCode || 502, safeParse(chunks)));
    }
  );
  req.on('error', () => sendJson(res, 502, rpcError(null, -32000, 'Upstream (production AbaPay) unreachable.')));
  req.write(bodyStr);
  req.end();
}
...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    switch (msg.method) {
      case 'initialize':
        return sendJson(res, 200, rpcResult(msg.id, { protocolVersion: PROTOCOL_VERSION, capabilities: { tools: {} }, serverInfo: SERVER_INFO }));
      case 'tools/list':
        return sendJson(res, 200, rpcResult(msg.id, { tools: TOOLS }));
      case 'tools/call':
        ...
        return proxyToProduction(body, res);
      default:
