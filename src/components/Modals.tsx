export function ReceiptModal({ receipt, isMainnet, onClose, onShare, onSupport }: any) {
  ...
  // Check if we need to display a PIN (Electricity or Education)
    ...
    <div className="fixed inset-0 z-[60] bg-slate-900/80 backdrop-blur-md flex justify-center items-center p-6 animate-in fade-in" onClick={onClose}>
       <div className="bg-white w-full max-w-sm rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="p-8 space-y-4">
             ...
             <div className="flex justify-between border-b border-slate-100 pb-3">
                <span className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">{isElectricity ? 'Meter Number' : receipt.service === 'Send Money' || receipt.service === 'Bank Transfer' ? 'Account No' : 'Recipient'}</span>
             ...
             {/* ⚡ UPDATED PIN DISPLAY LOGIC ⚡ */}
             {hasPin && (
               <div className="mt-4 bg-emerald-50 border-2 border-emerald-200 rounded-xl p-4 text-center">
                  <p className="text-[10px] font-black text-emerald-700 uppercase tracking-widest mb-1">{isElectricity ? 'Meter Token PIN' : 'Purchased PIN'}</p>
                  <p className="font-mono text-sm sm:text-base font-black text-slate-900 tracking-wide break-all">{receipt.purchased_code.replace(/token\s*[:\-]*\s*/gi, '').trim()}</p>
                  <p className="text-[9px] font-bold text-emerald-600 mt-2">{isElectricity ? 'Enter this exactly as shown into your meter.' : 'Keep this PIN safe and secure.'}</p>
               </div>
             )}
