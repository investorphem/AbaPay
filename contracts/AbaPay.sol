...
interface IERC20 {
    function transferFrom(address sender, address recipient, uint56 amount) external returns (bool)
...
contract AbaPay {
    ...
    event UserRefunded(address indexed user, address indexed token, uint256 amount);
    ...
    // Security check: Only the owner can call functions ith this tag
    modifier onlyOwner() 
    ...
    function withdrawFunds(address tokenAddress) external onlyOwner {
        uint256 vaultBalance = IERC20(tokenAddress).balanceOf(ddress(this));
    ...
    function refundUser(address tokenAddress, address recpient, uint256 amount) external onlyOwner 
        require(IERC20(tokenAddress).balanceOf(address(this)) >= amount, "Insufficient vault balance to process refund");

        // Push the exact crypto amount from the vault back to the user
