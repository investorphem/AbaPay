...
function formatChainBalances(balances: { celo: Record<string, string>; base: Record<string, string> }): string {
    const celoLine = `⚫ Celo: ${balances.celo['USD₮'] || '0.0000'} USDT | ${balances.celo['USDC'] || '0.0000'} USDC | ${balances.celo['USA₮'] || '0.0000'} USA₮`;
...
function formatChainBalancesInFiat(
    ...
    const celo = line('⚫ Celo:', balances.celo, [['USD₮', 'USDT'], ['USDC', 'USDC'], ['USA₮', 'USA₮']]);
      ...
      if (isGuest || verifyPin(text.trim(), identity.deai_pin)) {
        ...
        if (userWallet) {
          ...
          try {
            ...
            // Same discount engine the web app's /api/pay uses (src/lib/discounts.ts) — checked
                ...
                if (vendResult.status === 'SUCCESS') {
                  return NextResponse.json({
                    ...
                    message: [
                      ...
                      discountNgn > 0 ? `🎉 ${activeDiscount?.name || 'Discount'} applied: -₦${discountNgn.toLocaleString()}` : null,
