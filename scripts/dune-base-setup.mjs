...
/**
 * Retired: taken off the dashboard and ARCHIVED on Dune.
 *
 ...
 * queries are archived, and this script neither deploys nor runs them:
 *
 *   - excluded from the deploy loop — PATCHing an archived query would resurrect it
 ...
 *
 * To bring one back: unarchive it on Dune, give it a DESCRIPTIONS entry, add its chart to
 * the dashboard, and remove it from this set — all four, or it half-works.
 */
...
function writeIds(queries, { quiet = false } = {}) {
  ...
  const payload = {
    _comment:
      ...
      '(13_by_service, 14_by_contract, 15_by_rail) are archived on Dune and appear in `queries` ' +
